...
// [PySeqArray patch] native overload: resolve CFileInfo from a pygds file id
// directly (no R gds.class SEXP), via the pygds GDS_ID2FileRoot C-API.  Used by
// the SEXP-free native entry layer (csrc/native_api.cpp).  See tools/PATCHES.md.
COREARRAY_DLL_LOCAL CFileInfo &GetFileInfo(int file_id)
{
	PdGDSFolder root = GDS_ID2FileRoot(file_id);
	if (root == NULL)
		throw ErrSeqArray("Invalid GDS file id.");
